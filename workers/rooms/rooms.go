...
func main() {
	...
	if os.Getenv("GET_FROM_FILE") != "" {
		...
		err = json.Unmarshal(fileContent, &dataRoomDetails)
		if err != nil {
		...
		debug++
		utils.WriteLog(fmt.Sprintf("%s [GET_FROM_FILE] TOTAL_FETCH: %d DEBUG: %d; TIME: %s; TOTAL_TIME: %s;", logPrefix, len(dataRoomDetails), debug, time.Now().Sub(debugT), time.Now().Sub(tStart)), utils.LogLevelDebug)
		debugT = time.Now()
	...
	var errorMessages []models.RoomDetails
	var errorDuplicates []models.RoomDetails
	...
	for _, dataRoomDetail := range dataRoomDetails {
		...
		if err := dstDB.Create(&mRooms).Error; err != nil {
			...
			dataRoomDetail.Error = err.Error()
			if errCode, ok := err.(*pq.Error); ok {
				if errCode.Code == "23505" { //unique_violation
					errorDuplicates = append(errorDuplicates, dataRoomDetail)
					continue
				}
			}
			errorMessages = append(errorMessages, dataRoomDetail)
			continue
		}
		totalInsertedSQL++
	}
	debug++
	utils.WriteLog(fmt.Sprintf("%s [PSQL] TOTAL_INSERTED: %d; TOTAL_ERROR: %v DEBUG: %d; TIME: %s; TOTAL TIME EXECUTION: %s;", logPrefix, totalInsertedSQL, len(errorMessages), debug, time.Now().Sub(debugT), time.Now().Sub(tStart)), utils.LogLevelDebug)
