...
func main() {
	...
	if os.Getenv("GET_FROM_FILE") != "" {
		...
		if os.Getenv("START_DATE") != "" && os.Getenv("END_DATE") != "" {
			db = db.Where("chat_messages.created_at BETWEEN ? AND ?", os.Getenv("START_DATE"), os.Getenv("END_DATE"))
		} else if os.Getenv("START_DATE") != "" && os.Getenv("END_DATE") == "" {
			db = db.Where("chat_messages.created_at >=?", os.Getenv("START_DATE"))
		} else if os.Getenv("START_DATE") == "" && os.Getenv("END_DATE") != "" {
			db = db.Where("chat_messages.created_at <=?", os.Getenv("END_DATE"))
		}
