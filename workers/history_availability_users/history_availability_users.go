...
func main() {
	...
	var lists []models.HistoryAvailabilityAgent
	...
	var errorMessages []models.HistoryAvailabilityAgent
	var errorDuplicates []models.HistoryAvailabilityAgent
	totalInserted := 0
	var m models.HistoryAvailabilityUser

	for _, list := range lists {
		m.Id = list.Id
		m.CompanyId = list.CompanyId
		m.UserId = list.UserId
		m.Activity = list.Activity
		m.Reason = list.Reason
		m.AvailableTime = list.AvailableTime
		m.RequestTime = list.RequestTime
		m.UnAvailableTime = list.UnAvailableTime
		m.ReavailableTime = list.ReavailableTime
		m.Type = list.Type
		m.LoginId = list.MasterId
		m.CreatedAt = list.CreatedAt
		m.CreatedBy = list.CreatedBy
		m.UpdatedAt = list.CreatedAt
		m.UpdatedBy = list.CreatedBy
